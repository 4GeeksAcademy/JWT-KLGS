...
def get_favoritos():
    ...
    def CargarNombreEnBaseID(item): 
            ...
            valueSearch = 0
            for clave in item:
                if clave != "id" and clave != "user_id":
                    if item[clave] != None:
                        objetsearch = clave
                        valueSearch = item[clave]
            if objetsearch == "character_id":
                infoCharacter = Characters.query.filter_by(id=valueSearch).first()
                finalInfo = infoCharacter.serialize()
                return finalInfo["Name"]
            elif objetsearch == "planet_id":
                planetsinfo = Planets.query.filter_by(id=valueSearch).first()
                finalInfo = planetsinfo.serialize()
                return finalInfo["name"]
            elif objetsearch == "vehiculo_id":
                infovehicle = Vehicle.query.filter_by(id=valueSearch).first()
                finalInfo = infovehicle.serialize()
                return finalInfo["Modelo"]
            elif objetsearch == "film_id":
                infofilm = Films.query.filter_by(id=valueSearch).first()
                finalInfo = infofilm.serialize()
                return finalInfo["Title"]
            
    dataFinal = list(map(lambda item:CargarNombreEnBaseID(item),results))
    return jsonify({"msg":"estos son los favoritos del usuario","dataFinal":dataFinal}),200 
